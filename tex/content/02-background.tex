...
\begin{figure}[ht]
	\centering
	\input{figures/vispipeline.tikz}
...
\begin{figure}[ht]
	...
	\begin{tikzpicture}[line join=round]
		% the eye
		\draw [very thick, lower left=blue!50!white, upper right=white] (-2mm, 3mm) -- (-12mm, -3mm) -- (-1mm, -4mm);
		\shade [ball color=black] (-3mm, -1mm) circle [x radius=1.1mm, y radius=2.75mm, rotate=12];
		\node at (-8mm, 7mm) {Eye};
		
		\draw [->, very thick, >=stealth'] (0mm, 0mm) -- ++(12: 9cm);

		% Image Plane		
		\filldraw [thick, lower left=MyColor1!50!black, upper left=MyColor1, lower right=MyColor1, upper right=MyColor1!80!white, fill opacity=0.5]
			(15mm, 0mm) -- (29mm, -16mm) -- (30mm, 12mm) -- (14mm, 24mm) -- (15mm, 0mm);
		\draw [very thick] (0mm, 0mm) -- ++(12: 22mm);
		\filldraw [draw=black, fill=white] (12: 22mm)  circle (1.25mm);
		\node at (20mm, 28mm) {Image Plane};

		
		% Data Set
		\coordinate (p1) at (53mm, 17mm);
		\coordinate (p2) at (76mm, 22mm);
		\coordinate (p3) at (53mm, -7mm);
		\coordinate (p4) at (74mm, 1mm);
		
		\coordinate (p5) at (43mm, 27mm);
		\coordinate (p6) at (62mm, 30mm);
		\coordinate (p7) at (44mm,  6mm);
		\coordinate (p8) at (61.5mm, 11mm);
			
		\draw [] (p1) -- (p5) -- (p6) -- (p2) -- (p1);
		\draw [] (p6) -- (p8) -- (p4) -- (p3) -- (p7) -- (p8);
				
		\filldraw [thick, fill opacity=0.5, fill=MyColor2]
			(p1) -- (p2) -- (p4) -- (p3) -- (p1);
		\filldraw [thick, fill opacity=0.5, fill=MyColor2]
			(p1) -- (p5) -- (p7) -- (p3) -- (p1);
		\filldraw [thick, fill opacity=0.5, fill=MyColor2]
		...
		\node at (80mm, -4mm) {Data Set};


		% Sampling points
		\filldraw [draw=black, fill=white] (12: 50mm) circle (1.25mm);
		\filldraw [draw=black, fill=MyColor2!22!white] (12: 53mm) circle (1.25mm);
		\filldraw [draw=black, fill=MyColor2!22!white] (12: 56mm) circle (1.25mm);
		\filldraw [draw=black, fill=MyColor2!22!white] (12: 59mm) circle (1.25mm);
		\filldraw [draw=black, fill=MyColor2!22!white] (12: 62mm) circle (1.25mm);
		\filldraw [draw=black, fill=MyColor2!22!white] (12: 65mm) circle (1.25mm);
		\filldraw [draw=black, fill=MyColor2!22!white] (12: 68mm) circle (1.25mm);
		\filldraw [draw=black, fill=MyColor2!22!white] (12: 71mm) circle (1.25mm);
		
		% thick front outline of data set
		\draw [very thick] (p5) -- (p1) -- (p2) -- (p1) -- (p3);
	\end{tikzpicture}
	\caption{
		Illustration of image order based ray casting for direct volume rendering. 
		A set of rays is casted from the camera through the image plane towards the image data, where the data is sampled at a given frequency.
	}
	\label{fig:background:ray-casting-scheme}
\end{figure}
...
\begin{figure}[ht]
	\centering
	\subfloat[~Find the horizontal line. In this feature search, the orientation of the lines can be perceived preattentively, so that the outlier pops out almost immediately.]{
		\input{figures/background/featuresearch.tikz}
		\label{fig:background:FeatureSearch}
	}
	\qquad
	\subfloat[~Find the horizontal blue line. In this conjunction search, the outlier does no longer pop out, since the observer has to search for a combination of two features. This has to be performed in the focused attention stage.]{
		\input{figures/background/conjunctionsearch.tikz}
		\label{fig:background:ConjunctionSearch}
	}
	\caption{
...
\begin{figure}[ht]
	\centering
	\input{figures/background/responsetimes.tikz}
	\caption{
...
\begin{figure}[ht]
	\centering
	\subfloat[~Set of distinct colors for nominal data.]{
		\begin{tikzpicture}
			\fill [MyColor1] (0mm, 0mm) circle (4mm);
			\fill [MyColor2] (10mm, 0mm) circle (4mm);
			\fill [MyColor3] (20mm, 0mm) circle (4mm);
			\fill [MyColor4] (0mm, -10mm) circle (4mm);
			\fill [MyColor5] (10mm, -10mm) circle (4mm);
			\fill [MyColor6] (20mm, -10mm) circle (4mm);
		\end{tikzpicture}
	}
	\qquad
	\subfloat[~Heated color map as example for a luminance contrast color map.]{
		\begin{tikzpicture}
			\fill [left color=black, right color=red] (0mm, 0mm) rectangle (15mm, 10mm);
			\fill [left color=red, right color=yellow] (15mm, 0mm) rectangle (40mm, 10mm);
			\draw [white] (0mm, -4mm) -- (40mm, -4mm);
		\end{tikzpicture}
	}
	\qquad
	\subfloat[~Diverging cold-hot color map that can be used for two-ended scales.]{
		\begin{tikzpicture}
			\fill [left color=blue, right color=gray!10] (0mm, 0mm) rectangle (20mm, 10mm);
			\fill [left color=gray!10, right color=red] (20mm, 0mm) rectangle (40mm, 10mm);
			\draw [white] (0mm, -4mm) -- (40mm, -4mm);
		\end{tikzpicture}
	}
	\caption{
		\TODO{Illustration of different color maps.}
	}
	\label{fig:background:color-maps}
\end{figure}
...
\begin{figure}[ht]
	\centering
	\input{figures/background/munzner-nestedmodel1.tikz}
	\caption{
...
\begin{figure}[ht]
	\centering
	\input{figures/background/munzner-nestedmodel2.tikz}
	\caption{
